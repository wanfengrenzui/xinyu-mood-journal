...
function switchPage(page) {
  ...
  ["today","plan","journey","care","about"].forEach((name)=>$(`#${name}Page`).classList.toggle("active",name===page));
  if(page==="plan"){renderPlan();setFlowStep(5)}
...
function savePracticeFeedback(type,before,after) {
  const feedback=careFeedback();feedback[type]=feedback[type]||{better:0,same:0,worse:0,totalDelta:0};const result=after<before?"better":after===before?"same":"worse";feedback[type][result]++;feedback[type].totalDelta+=(before-after);localStorage.setItem(FEEDBACK_KEY,JSON.stringify(feedback));const plan=currentPlan();if(plan){const now=plan.steps.find((step)=>step.id==="now");if(now&&now.care===type)now.done=true;plan.adjustment=result==="better"?"首个练习有效，保留后续轻量节点":result==="same"?"首个练习无明显变化，后续建议保持克制":"首个练习后感受更强，已降低该方法优先级";localStorage.setItem(PLAN_KEY,JSON.stringify(plan));renderPlan()}setFlowStep(4);
...
function openProductTour(){openModal(`<p class="step-label">面试官建议路径 · 约 1 分钟</p><h2 id="modalTitle">看见产品如何从记录走向持续关怀</h2><div class="tour-steps"><div><b>1</b><span>一键填入完整记录，生成事实/推测/历史/建议四层洞察</span></div><div><b>2</b><span>查看当下、今晚、明日三阶段关怀计划</span></div><div><b>3</b><span>完成练习并反馈前后强度，让推荐发生变化</span></div><div><b>4</b><span>在轨迹和关怀库查看置信度、证据链与个性化结果</span></div></div><button class="primary-button compact" id="startTour">开始完整体验</button>`);$("#startTour").addEventListener("click",()=>{closeModal();switchPage("today");fillDemo();showToast("第 1 步：生成本次洞察")})}
